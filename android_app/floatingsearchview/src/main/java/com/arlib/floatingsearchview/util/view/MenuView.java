    ...
    public void reset(int menu, int availWidth) {
        ...
        if (availItemRoom > 0) {
            for (int i = 0; i < localActionItems.size(); i++) {
                ...
                if(menuItem.getActionView()!=null || menuItem.getIcon() != null){
                  View action= null;
                    if(menuItem.getActionView()!=null){
                        action= menuItem.getActionView();
                    }else if (menuItem.getIcon() != null) {
                        ImageView a = createActionView();
                        a.setContentDescription(menuItem.getTitle());
                        a.setImageDrawable(menuItem.getIcon());
                        Util.setIconColor(a, mActionIconColor);
                        action= a;
                    }
                    addView(action);
    ...
    public void showIfRoomItems(boolean withAnim) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (i < mActionItems.size()) {
                ...
                if(action instanceof ImageView){
                    ((ImageView)action).setImageDrawable(actionItem.getIcon());
                    Util.setIconColor((ImageView)action, mActionIconColor);
                }
                action.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
